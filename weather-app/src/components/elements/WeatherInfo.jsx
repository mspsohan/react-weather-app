    ...
    return (
        <>
            <div className="temperature">
                {unit ? convertF(data?.daily[0]?.temp?.eve)?.toFixed(0) : convertC(data?.daily[0]?.temp?.eve)?.toFixed(0)}<span className="degree">°</span> <span className="unit">{unit ? "F" : "C"}</span>
            </div>
            <div className="date">
            ...
            <div className="info-weather">
                <div>
                    ...
                    <span>Feels like {unit ? convertF(data?.daily[0]?.feels_like?.eve).toFixed(0) : convertC(data?.daily[0]?.feels_like?.eve)?.toFixed(0)} ° {unit ? "F" : "C"}</span>
                </div>
                {data?.clouds && (<div>
                    <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg" data-reactroot="">
